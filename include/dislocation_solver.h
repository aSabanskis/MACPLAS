...
private:
  ...
  /** Calculate the temperature and stress-dependent Peierls potential \f$Q\f$
   */
  double
  calc_Q(const double T, const double tau_eff) const;
  ...
  /** Peierls potential correction \f$dQ\f$ (stress function), eV
   */
  FunctionParser<1> m_dQ;
  ...
  , previous_time_step(0)
{
  ...
  prm.declare_entry("Peierls potential correction",
                    "0",
                    Patterns::Anything(),
                    "Change of Peierls potential Q in eV (stress function)");
...
template <int dim>
void
DislocationSolver<dim>::get_boundary_points(const unsigned int       id,
                                            std::vector<Point<dim>> &points,
                                            IndexSet &boundary_dofs) const
{
...
DislocationSolver<dim>::output_vtk() const
{
  ...
  output_data_vector(T, "T", data_out);

  output_data_vector(N_m, "N_m", data_out);

  const Vector<double> tau = tau_eff(N_m, J_2, T);

  Vector<double> Q(T.size());
  for (unsigned int i = 0; i < T.size(); ++i)
    Q[i] = calc_Q(T[i], tau[i]);
  output_data_vector(Q, "Q", data_out);

  Vector<double> D(T.size());
  for (unsigned int i = 0; i < T.size(); ++i)
    D[i] = calc_D(T[i]);
  output_data_vector(D, "D", data_out);

  Vector<double> tau_crit(T.size());
  for (unsigned int i = 0; i < T.size(); ++i)
    tau_crit[i] = calc_tau_crit(T[i]);
  output_data_vector(tau_crit, "tau_crit", data_out);
  ...
  for (unsigned int i = 0; i < epsilon_c.n_blocks(); ++i)
    {
      ...
      output_data_vector(epsilon_c.block(i), name, data_out);
    }
  ...
  output_data_vector(J_2, "stress_J_2", data_out);
  output_data_vector(tau, "tau_eff", data_out);
  output_data_vector(dot_N_m, "dot_N_m", data_out);
  output_data_vector(v, "v", data_out);

  for (unsigned int i = 0; i < dot_epsilon_c.n_blocks(); ++i)
    {
      const std::string name = "dot_epsilon_c_" + std::to_string(i);
      output_data_vector(dot_epsilon_c.block(i), name, data_out);
    }
...
DislocationSolver<dim>::initialize_parameters()
{
  ...
  const std::string m_dQ_expression = prm.get("Peierls potential correction");
  m_dQ.initialize("tau",
                  m_dQ_expression,
...
template <int dim>
double
DislocationSolver<dim>::calc_Q(const double T, const double tau_eff) const
{
  return m_Q.value(Point<1>(T)) + m_dQ.value(Point<1>(tau_eff));
}
                                          ...
                                          const double S) const
{
  ...
  return m_b * m_k_0 * N_m * std::pow(tau, m_p) *
         std::exp(-calc_Q(T, tau) / (m_k_B * T)) * S /
         (2 * m_F * std::sqrt(J_2));
}
                                             ...
                                             const double T) const
{
  ...
  return m_k_0 * std::pow(tau, m_p) * std::exp(-calc_Q(T, tau) / (m_k_B * T));
}
